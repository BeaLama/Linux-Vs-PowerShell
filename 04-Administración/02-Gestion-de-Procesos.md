...
- Mantener la prioridad normal en la mayoría de aplicaciones.
- Modificar prioridades únicamente cuando sea necesario.
- Supervisar el rendimiento tras realizar cambios.
- Evitar utilizar prioridad de tiempo real salvo en casos muy concretos.
...
- Utilizar cuentas de servicio con permisos mínimos.
- Revisar periódicamente los servicios configurados para iniciarse automáticamente.
...
- Identificar la causa del bloqueo antes de finalizar el proceso.
- Utilizar herramientas de monitorización para comprobar el consumo de recursos.
- Finalizar procesos de forma controlada.
- Evitar el uso de **SIGKILL** salvo que sea imprescindible.
- Revisar los registros del sistema si el problema se repite.
...
- Mantener únicamente los procesos necesarios.
- Revisar los programas que se inician automáticamente.
- Comprobar el consumo de recursos.
- Identificar procesos desconocidos.
- Analizar cualquier proceso sospechoso antes de finalizarlo.
...
- Registrar los resultados mediante logs.
- Utilizar cuentas con permisos mínimos.
...
- Conservar los logs durante el tiempo necesario.
- Automatizar la supervisión cuando sea posible.
- Registrar únicamente la información necesaria.
- Proteger los registros frente a modificaciones no autorizadas.
...
## Buenas prácticas en la gestión de procesos

Introducción

Una correcta gestión de los procesos contribuye a mantener el sistema operativo estable, seguro y con un rendimiento óptimo. Aunque los sistemas modernos administran automáticamente la mayoría de los procesos, es responsabilidad del administrador supervisar su funcionamiento, detectar anomalías y aplicar medidas preventivas cuando sea necesario.

Seguir una serie de buenas prácticas reduce el riesgo de incidencias, facilita el diagnóstico de problemas y mejora la eficiencia en la administración de equipos y servidores.

---

# Supervisar periódicamente los procesos

Es recomendable revisar de forma periódica los procesos que se encuentran en ejecución.
...
La supervisión periódica ayuda a detectar problemas antes de que afecten al funcionamiento del sistema.

---

# No finalizar procesos sin conocer su función

Antes de detener un proceso es importante identificar su finalidad.

Finalizar un proceso crítico puede provocar:
...
Siempre que exista alguna duda, es recomendable investigar primero el proceso antes de detenerlo.

---

# Revisar el consumo de recursos

Un proceso que consume demasiada CPU, memoria o disco puede afectar al rendimiento general del equipo.

Es aconsejable comprobar periódicamente:
...
Si un proceso presenta un consumo anormal de forma continuada, conviene analizar su comportamiento.
...
- Deshabilitar programas que se inician automáticamente sin necesidad.

Reducir el número de procesos activos mejora el rendimiento del sistema.

---

# Revisar los programas de inicio

Muchas aplicaciones se configuran para iniciarse automáticamente junto con el sistema operativo.

Es recomendable revisar periódicamente estos programas y deshabilitar aquellos que no sean necesarios.

Esto reduce el tiempo de arranque y disminuye el consumo de recursos desde el inicio del sistema.

---

# Mantener el sistema actualizado

Las actualizaciones corrigen errores y mejoran el funcionamiento de los procesos del sistema.

Se recomienda mantener actualizados:
...
Además de mejorar el rendimiento, las actualizaciones corrigen vulnerabilidades de seguridad.

---

# Automatizar tareas repetitivas

Las tareas administrativas que se realizan con frecuencia pueden automatizarse mediante scripts o tareas programadas.
...
Los servicios también son procesos que deben revisarse periódicamente.
...
Los servicios innecesarios deberían deshabilitarse para mejorar el rendimiento y reducir la superficie de ataque.

---

# Analizar procesos desconocidos

Si aparece un proceso cuyo origen no se conoce, es recomendable:

- Comprobar su nombre.
- Revisar su ubicación.
- Verificar su firma digital (Windows).
- Consultar la documentación oficial.
- Analizar el archivo con un antivirus.

Nunca debe asumirse que un proceso es seguro únicamente por estar en ejecución.

---

# Utilizar herramientas adecuadas

Cada sistema operativo dispone de herramientas específicas para la gestión de procesos.
...
Utilizar la herramienta adecuada facilita el diagnóstico y la administración del sistema.

---

# Documentar las incidencias

Cuando se produzca un problema relacionado con un proceso, es recomendable registrar:
...
La documentación facilita futuras intervenciones y ayuda a identificar incidencias recurrentes.
...
Este procedimiento permite actuar de forma ordenada y reducir el tiempo necesario para resolver el problema.
...
Los conocimientos sobre gestión de procesos cobran especial importancia cuando se aplican a situaciones reales. En el trabajo diario de un administrador de sistemas es habitual tener que identificar procesos problemáticos, solucionar bloqueos o analizar consumos anómalos de recursos.

Los siguientes casos prácticos muestran procedimientos habituales para diagnosticar y resolver incidencias relacionadas con los procesos tanto en Windows como en Linux.

---

# Caso práctico 1: Aplicación bloqueada en Windows

## Situación

Un usuario informa de que Microsoft Word ha dejado de responder y no puede cerrarlo de forma normal.
...
Si el problema se repite con frecuencia, será necesario revisar:
...
# Caso práctico 2: Proceso consumiendo demasiada CPU

## Situación

El equipo funciona con mucha lentitud y el ventilador permanece funcionando constantemente.
...
Analizar si es un proceso habitual

↓

Reiniciar o finalizar si procede
```
...
# Caso práctico 3: Consumo elevado de memoria

## Situación

El sistema comienza a utilizar memoria virtual y las aplicaciones responden lentamente.
...
Tras localizar el proceso, se debe analizar si el consumo es normal o si existe una fuga de memoria (*Memory Leak*).

---

# Caso práctico 4: Servicio que no responde

## Situación

Un servidor web deja de responder a las peticiones de los usuarios.
...
Si el servicio continúa fallando, se revisarán los registros del sistema.

---

# Caso práctico 5: Identificar un proceso sospechoso

## Situación

Aparece un proceso desconocido consumiendo recursos del sistema.
...
No se recomienda finalizar un proceso sin conocer previamente su función.

---

# Caso práctico 6: Proceso que impide apagar el equipo

## Situación

Windows informa de que una aplicación impide el apagado del sistema.
...
Forzar el cierre únicamente cuando sea imprescindible.

---

# Caso práctico 7: Analizar un proceso desde PowerShell

## Situación

Se desea obtener información sobre los procesos que más memoria consumen.
...
Para mostrar únicamente los primeros resultados:
...
# Caso práctico 8: Finalizar un proceso en Linux

## Situación

Un proceso ha dejado de responder.
...
Se recomienda utilizar primero `kill` y recurrir a `kill -9` únicamente cuando sea necesario.

---

# Caso práctico 9: Automatizar una comprobación diaria

## Situación

El administrador desea generar diariamente un listado de procesos en ejecución.
...
Programarlo mediante el **Programador de tareas**.
